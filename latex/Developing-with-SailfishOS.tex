...
All mentioned trademarks and trade names are the property of their respective holders, I have not marked them with a \texttrademark, \textregistered \hspace{4 pt} or \copyright \hspace{4 pt} sign. Use some common sense here. Not all information is written by myself, I've tried to quote as responsible as possible and quite intensive if appropiate. Rad this notes as a human being, not like a lawyer.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.32]{./gfx/vboxdownload.png} 
...
\subsubsection{kits}
%
But before we do that, we must talk about \textit{kits}. A kit is kind of an umbrella setting, which combines the information of the following bits and pieces, like \verb,qmake,, \verb,compiler,, and \verb,device type,. When we try to do some of the steps manually via command line, we will see that it often does not work but the same action succeeds invoked by QtCreator. One of the reasons are those kits. This is the information hub that QtCreator uses to pull all information together and initiate its actions.
\begin{figure}[H]
  \centering
...
%
Developing with the emulator only will do you no good. You have to experience your program on a real device. Things that might look great on an emulator, may not even work for you on a real phone. It maybe just you fingers that are hiding the screen.
